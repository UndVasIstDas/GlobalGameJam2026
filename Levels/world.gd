...
func _process(delta: float) -> void:
	...
	if(module_queue.size() < MODULE_LIMIT):
		...
		module_pos_queue.push_front(module_pos_queue[0]+tilemap.get_used_rect().size.x*tilemap.tile_set.tile_size.x)
	...
	#Unload scenes
	if(get_node("Player").global_position.x > module_pos_queue[module_pos_queue.size()-1]):
